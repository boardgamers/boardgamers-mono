...
- coyo: nginx vhosts `sites-available/{pr-preview,pr-preview-api}`, wildcard cert
  ...
  - The `pr-preview` vhost carries `gzip on; gzip_proxied any; …` (#127) so SSR
    HTML and API JSON are compressed even though the env app servers also
    compress. When syncing `coyo-pr-preview.nginx.conf` to coyo:
