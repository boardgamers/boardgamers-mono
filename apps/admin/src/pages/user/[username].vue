...
import { IUser, ApiError } from "@bgs/models";
...
<template>
  <div>
    <v-card :loading="loading">
      ...
      <v-text-field v-if="user" v-model.number="user.account.karma" label="Karma" type="number" @change="updateKarma" />
      <template #actions>
        <v-btn :disabled="user?.security?.confirmed" @click="confirmUser"> Confirm user </v-btn>
        <v-btn @click="login"> Log in as </v-btn>
      </template>
    </v-card>
    <v-card class="mt-4" :loading="loading">
      <template #title> Boardgame management </template>
    ...
    <v-card class="mt-4">
      <template #title> Api Errors </template>
      <v-data-table
        :headers="apiHeaders"
        :items="apiErrors"
        item-key="_id"
