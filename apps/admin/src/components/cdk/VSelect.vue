...
const props = defineProps<{
  ...
  multiple?: boolean;
  required?: boolean;
  items: Array<{ text: string; value: string | null }>;
}>();

const nullProp = `__VSELECT__NULL__PROP`;
...
function getInput(elem: HTMLSelectElement) {
  if (!props.multiple) {
    return elem.value === nullProp ? null : elem.value;
  }

  return new Array(elem.children.length)
    .fill(0)
    .map((_, i) => elem.children[i] as HTMLOptionElement)
    .filter((elem) => elem.selected)
    .map((elem) => valueToSrc.value[elem.value]);
}
</script>
