...
`env.listen.host` defaults to `127.0.0.1` (was `localhost`). On hosts whose `/etc/hosts` maps `localhost` to **both** `127.0.0.1` and `::1`, Node's `http.listen("localhost")` binds **only `::1`**, while nginx (prod) and the Vite proxy (dev) dial `127.0.0.1` → `ECONNREFUSED`. The same bug already forced `127.0.0.1` in `app/config/test-setup.ts`; this just extends that fix to normal startup. Prod is unaffected or improved (nginx upstream is already `127.0.0.1:50801` — `apps/api/app/config/nginx:2`), and operators can still force `::1` / `0.0.0.0` via `listenHost`. Game-server (`apps/game-server/app/config/env.ts`) mirrors this. Revisit if we ever move to dual-stack listen or a hostname-based upstream.
