...
import { z } from "zod";
import { canUserManageGame, isPublicChatRoom, playerOrderSchema, type GameDoc, type UserDoc } from "@bgs/models";
...
// Re-exported for existing importers (specs, web mirrors the constant separately).
export { CHAT_EDIT_WINDOW_MS } from "../chat-handlers.ts";
...
// The chat handlers themselves are room-generic (shared with the public lobby
// rooms, see ../chat-handlers.ts); the game routes only add the game-specific
// auth (participants + game admins) before delegating.
