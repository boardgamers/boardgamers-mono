// The vote-cancel POST answers 204 (no body): the button must disable immediately on
// success (local vote applied) and stay enabled on failure. Real GameSidebar, jsdom env.
...
// The in-place `voteCancel` mutation only re-renders through a `$state` proxy (as in the real layout).
