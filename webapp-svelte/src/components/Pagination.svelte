<script lang="ts">
  ...
  $: classes= classnames(align ? ['d-flex', `justify-content-${{right: 'end', center: 'center', left: 'start'}[align]}`] : '', className)
  ...
  function pageFor(position: number) {
    const pos = Math.min(Math.max(position, currentPage + (position - (pageItems-1)/2)), totalPages + position - pageItems)
